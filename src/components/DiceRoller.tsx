...
// Hoisted static dot pattern lookup map to prevent re-creation on every render/lookup call
...
// Hoisted static grid indices array to avoid allocating [...Array(9)] on every render
const GRID_INDICES = [0, 1, 2, 3, 4, 5, 6, 7, 8];

const DiceFace = ({ value, rolling }: { value: number; rolling: boolean }) => {
  // Fetch dot pattern once per render instead of 9 times in the map loop
