...
export const GameBoard = () => {
  ...
  // Optimization: Pre-calculate property owners and player positions to avoid O(N) searches during render.
  // This reduces complexity from O(Cells * Players) to O(Cells + Players).
  ...
  const playersOnCells = useMemo(() => {
    ...
    gameState?.players.forEach(player => {
      if (!player.bankrupt) {
        const players = playersMap.get(player.position) || [];
        players.push(player);
        playersMap.set(player.position, players);
      }
    });
    return playersMap;
  }, [gameState]);
