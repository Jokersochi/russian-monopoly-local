...
export const GameBoard = () => {
  ...
  // Memoize property ownership, player positions, and standings to avoid O(P * C) lookups per render frame
  const { ownerByCellId, playersByCellId, standings } = useMemo(() => {
    if (!gameState) {
      return { ownerByCellId: new Map<number, NonNullable<typeof gameState>['players'][0]>(), playersByCellId: new Map<number, NonNullable<typeof gameState>['players']>(), standings: [] };
    }
    const ownerMap = new Map<number, typeof gameState.players[0]>();
    const playersMap = new Map<number, typeof gameState.players>();

    gameState.players.forEach((p) => {
      p.properties.forEach((cellId) => ownerMap.set(cellId, p));
      const list = playersMap.get(p.position) || [];
      list.push(p);
      playersMap.set(p.position, list);
    });

    const calculatedStandings = gameState.players
      .map((p) => {
        let propVal = 0;
        let houseVal = 0;
        p.properties.forEach((propId) => {
          const c = cells[propId]?.id === propId ? cells[propId] : cells.find((cell) => cell.id === propId);
          if (c) {
            propVal += c.price || 0;
            houseVal += (gameState.houses[c.id] || 0) * (c.houseCost || 0);
          }
        });
    ...
    return { ownerByCellId: ownerMap, playersByCellId: playersMap, standings: calculatedStandings };
  }, [gameState, cells]);
