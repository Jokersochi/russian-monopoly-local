...
export const GameBoard = () => {
  ...
  const playerPositionsKey = players.map((p) => `${p.id}:${p.position}`).join(',');
  ...
  // Pre-compute players on each cell: Map<cellId, Player[]>
  ...
  // Pre-compute net worth standings (O(P * K) fast path instead of O(P * C) array filtering)
