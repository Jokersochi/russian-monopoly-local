...
export const GameBoard = () => {
  ...
  // Memoize property ownership, player positions, and net worth standings to optimize render loops from O(P * C) to O(1) Map lookups
  const { propertyOwners, playersOnCellMap, sortedStandings } = useMemo(() => {
    const ownersMap = new Map<number, NonNullable<typeof players>[0]>();
    const playersOnCellMap = new Map<number, NonNullable<typeof players>>();

    for (let i = 0; i < players.length; i++) {
      const player = players[i];

      const posList = playersOnCellMap.get(player.position);
      if (posList) {
        posList.push(player);
      } else {
      ...
      for (let j = 0; j < player.properties.length; j++) {
        ownersMap.set(player.properties[j], player);
      }
    }
    ...
    return { propertyOwners: ownersMap, playersOnCellMap, sortedStandings: standings };
