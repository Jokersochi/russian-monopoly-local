...
export const GameBoard = () => {
  ...
  // Performance Optimization: Precalculate owner map, players on cell map, and net worth standings
  // inside a single top-level useMemo pass to avoid O(P * C) lookups on every cell render frame.
  const { ownerMap, playersOnCellMap, netWorthStandings } = useMemo(() => {
    const ownerMap = new Map<number, Player>();
    const playersOnCellMap = new Map<number, Player[]>();
    ...
    for (const p of players) {
      ...
      const existing = playersOnCellMap.get(p.position);
      if (existing) {
        existing.push(p);
      } else {
        playersOnCellMap.set(p.position, [p]);
      }
    }
    ...
    return { ownerMap, playersOnCellMap, netWorthStandings: standings };
  }, [gameState, cells]);
  ...
  const pname = (p: NonNullable<typeof players>[0]) => p.displayName || t(`players.${p.nameKey}`);
