...
export const GameBoard = () => {
  ...
  // Memoize player positions for stable effect dependency
  const playerPositionsStr = JSON.stringify(gameState?.players.map(p => p.position));
  ...
  }, [gameState, playerPositionsStr]);

  // Memoize cell data lookups to optimize O(Cells * Players) render complexity to O(Cells + Players)
  const propertyOwners = useMemo(() => {
    if (!gameState) return new Map<number, typeof gameState.players[0]>();
    const owners = new Map<number, typeof gameState.players[0]>();
    gameState.players.forEach(p => {
      p.properties.forEach(pid => owners.set(pid, p));
    });
    return owners;
  }, [gameState]);

  const playersOnCells = useMemo(() => {
    if (!gameState) return new Map<number, typeof gameState.players>();
    const mapping = new Map<number, typeof gameState.players>();
    gameState.players.forEach(p => {
      const list = mapping.get(p.position) || [];
      mapping.set(p.position, [...list, p]);
    });
    return mapping;
  }, [gameState]);

  // Memoize standings calculation to avoid O(Players * Cells) complexity in render
  const standings = useMemo(() => {
    if (!gameState) return [];
    return [...gameState.players]
      .map((p) => {
        // Use direct cell indexing (O(1)) instead of filter (O(Cells))
        const propVal = p.properties.reduce((sum, pid) => sum + (cells[pid]?.price || 0), 0);
        const houseVal = p.properties.reduce((sum, pid) => sum + (gameState.houses[pid] || 0) * (cells[pid]?.houseCost || 0), 0);
