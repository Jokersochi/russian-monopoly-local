...
export const GameBoard = () => {
  ...
  // Safely destructure gameState before conditional return to comply with Rules of Hooks
  const { players = [], currentPlayer: cpIdx = 0, houses = {} } = gameState || {};

  // Optimization: Pre-calculate cell owner and player location maps in O(P) time instead of O(C * P) find/filter per cell.
  const { ownerByCellId, playersByCellId } = useMemo(() => {
    const ownerMap: Record<number, typeof players[0]> = {};
    const playersMap: Record<number, typeof players> = {};
  ...
  // Optimization: Memoize net worth calculations and sorting to avoid O(P * C) filtering and sorting on every render frame.
  const netWorthStandings = useMemo(() => {
    ...
    return [...players]
      .map((p, i) => {
        ...
        let houseVal = 0;
        p.properties.forEach((cellId) => {
          const cell = cells[cellId]?.id === cellId ? cells[cellId] : cells.find((c) => c.id === cellId);
          if (cell) {
  ...
  }, [players, cells, houses, gameState]);
