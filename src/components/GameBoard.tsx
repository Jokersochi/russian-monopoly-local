...
export const GameBoard = () => {
  ...
  // Stable key for player positions to avoid inline array mapping/joining in useEffect
  const playerPositionsKey = useMemo(
    () => gameState?.players.map(p => `${p.id}:${p.position}`).join(',') ?? '',
    [gameState]
  );

  // Precompute owner lookup per cell ID for O(1) access during board rendering
  ...
  // Precompute players present per cell ID for O(1) access during board rendering
  ...
  // Memoize net worth calculations and ranking to avoid filtering all cells per player on every render frame
