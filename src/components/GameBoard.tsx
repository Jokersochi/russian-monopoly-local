...
export const GameBoard = () => {
  ...
  // Optimize ownership lookups, player cell positions, and net worth standings in a single O(P * K) pass
  const { propertyOwners, playersOnCells, standings } = useMemo(() => {
    ...
    if (!gameState) {
      return { propertyOwners: ownerMap, playersOnCells: playersOnCellMap, standings: [] };
    }
    ...
    const playerStats = players.map((p) => {
      ...
      const onCell = playersOnCellMap.get(p.position) || [];
      onCell.push(p);
      playersOnCellMap.set(p.position, onCell);

      for (let i = 0; i < p.properties.length; i++) {
        const pid = p.properties[i];
        ownerMap.set(pid, p);
    ...
    playerStats.sort((a, b) => b.net - a.net);

    return {
      propertyOwners: ownerMap,
      playersOnCells: playersOnCellMap,
