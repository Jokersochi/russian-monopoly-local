...
export const GameBoard = () => {
  ...
  // Precalculate cell ownership lookups, player position groupings, and net worth standings
  // in a single top-level useMemo hook to avoid O(P * C) linear searches on every render.
  const { ownerByCellId, playersByCellId, sortedStandings } = useMemo(() => {
    if (!gameState) {
      return { ownerByCellId: new Map<number, Player>(), playersByCellId: new Map<number, Player[]>(), sortedStandings: [] };
    }
    const { players = [], houses = {} } = gameState;
    const ownerMap = new Map<number, Player>();
    const playersMap = new Map<number, Player[]>();
    const propVal = new Map<number, number>();
    const houseVal = new Map<number, number>();

    for (const p of players) {
      const onCell = playersMap.get(p.position);
      if (onCell) {
        onCell.push(p);
      } else {
        playersMap.set(p.position, [p]);
      }

      for (const propId of p.properties) {
        ownerMap.set(propId, p);
        const cell = cells[propId]?.id === propId ? cells[propId] : cells.find(c => c.id === propId);
        if (cell) {
          propVal.set(p.id, (propVal.get(p.id) || 0) + (cell.price || 0));
          const hc = houses[propId] || 0;
          houseVal.set(p.id, (houseVal.get(p.id) || 0) + hc * (cell.houseCost || 0));
        }
      }
    }
    ...
    const standings = players
      .map((p) => ({
        p,
        net: p.money + (propVal.get(p.id) || 0) + (houseVal.get(p.id) || 0),
      }))
      .sort((a, b) => b.net - a.net);

    return { ownerByCellId: ownerMap, playersByCellId: playersMap, sortedStandings: standings };
  }, [gameState, cells]);
