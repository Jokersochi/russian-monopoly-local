...
export const GameBoard = () => {
  ...
  // Precalculate cell ownership, player cell positions, and net worth standings to optimize render complexity from O(P*C) to O(1)
  ...
  const { ownerMap, playersOnCellMap, netWorthStandings } = useMemo(() => {
    if (!gameState) {
      return {
        ownerMap: new Map<number, (typeof players)[0]>(),
        playersOnCellMap: new Map<number, typeof players>(),
        netWorthStandings: [],
      };
    }

    const ownerMap = new Map<number, (typeof players)[0]>();
    const playersOnCellMap = new Map<number, typeof players>();

    players.forEach((p) => {
      p.properties.forEach((cellId) => ownerMap.set(cellId, p));
      const list = playersOnCellMap.get(p.position) || [];
      list.push(p);
      playersOnCellMap.set(p.position, list);
    });

    const netWorthStandings = players
      .map((p, i) => {
        let propVal = 0;
        let houseVal = 0;
  ...
  }, [gameState, cells, players, houses]);
