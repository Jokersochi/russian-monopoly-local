...
export const GameBoard = () => {
  ...
  // Precalculate cell lookups and net worth standings to replace O(P * C) search per render frame with O(1) Map lookups
  const { ownerMap, playersOnCellMap, netWorthStandings } = useMemo(() => {
    if (!gameState) {
      return {
        ownerMap: new Map<number, NonNullable<typeof gameState>['players'][0]>(),
        playersOnCellMap: new Map<number, NonNullable<typeof gameState>['players'][0][]>(),
        netWorthStandings: [],
      };
    }

    const ownerMap = new Map<number, NonNullable<typeof gameState>['players'][0]>();
    const playersOnCellMap = new Map<number, NonNullable<typeof gameState>['players'][0][]>();

    for (const player of gameState.players) {
      for (const propId of player.properties) {
        ownerMap.set(propId, player);
      }
      const existing = playersOnCellMap.get(player.position);
      if (existing) {
        existing.push(player);
      } else {
        playersOnCellMap.set(player.position, [player]);
      }
    }

    const netWorthStandings = gameState.players
      .map((p) => {
        let propVal = 0;
        let houseVal = 0;
        for (const pid of p.properties) {
          const c = cells[pid]?.id === pid ? cells[pid] : cells.find(cell => cell.id === pid);
          if (c) {
            propVal += c.price || 0;
            const hc = gameState.houses[pid] || 0;
            houseVal += hc * (c.houseCost || 0);
          }
        }
    ...
    return { ownerMap, playersOnCellMap, netWorthStandings };
  }, [gameState, cells]);
    ...
    <>
      <div className="relative bg-gradient-board rounded-2xl shadow-board p-8 border-8 border-russia-gold backdrop-blur-sm">
        ...
        <div className="relative" style={{ width: '902px', height: '902px' }}>
          {cells.map((cell) => {
            const owner = ownerMap.get(cell.id);
