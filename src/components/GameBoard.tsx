...
export const GameBoard = () => {
  ...
  // Optimization: Pre-calculate Map lookups to avoid O(N*M) searches in the 40-cell render loop.
  // This reduces per-cell complexity from O(Players + Properties) to O(1).
  const propertyOwners = useMemo(() => {
    const ownersMap = new Map<number, any>();
    if (!gameState) return ownersMap;
    gameState.players.forEach(player => {
      player.properties.forEach(propId => {
        ownersMap.set(propId, player);
      });
    });
    return ownersMap;
  }, [gameState]);
  ...
  const playerPositions = useMemo(() => {
    ...
    if (!gameState) return positionsMap;
    gameState.players.forEach(player => {
