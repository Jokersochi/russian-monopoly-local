...
export const GameBoard = () => {
  ...
  // Precompute cell owners, players on each cell, and net worth standings in a single pass O(P + C)
  const { cellOwners, playersOnCell, standings } = useMemo(() => {
    const owners = new Map<number, typeof players[0]>();
    const cellPlayers = new Map<number, typeof players[0][]>();
    const playerNetWorths = players.map((p) => ({ p, net: p.money }));

    const playerIndexMap = new Map<number, number>();
    players.forEach((p, idx) => {
      playerIndexMap.set(p.id, idx);
      const list = cellPlayers.get(p.position) || [];
      list.push(p);
      cellPlayers.set(p.position, list);
    });

    cells.forEach((c) => {
      for (const p of players) {
        if (p.properties.includes(c.id)) {
          owners.set(c.id, p);
          const pIdx = playerIndexMap.get(p.id);
          if (pIdx !== undefined) {
            const houseCount = houses[c.id] || 0;
            playerNetWorths[pIdx].net += (c.price || 0) + houseCount * (c.houseCost || 0);
          }
          break;
        }
      }
    ...
    playerNetWorths.sort((a, b) => b.net - a.net);

    return {
      cellOwners: owners,
      playersOnCell: cellPlayers,
      standings: playerNetWorths,
    };
  }, [cells, players, houses]);
  ...
  const pname = (p: typeof players[0]) => p?.displayName || (p ? t(`players.${p.nameKey}`) : '');
      ...
      <div className="relative bg-gradient-board rounded-2xl shadow-board p-8 border-8 border-russia-gold backdrop-blur-sm">
        ...
        <div className="relative" style={{ width: '902px', height: '902px' }}>
          {cells.map((cell) => {
            ...
            const playersHere = playersOnCell.get(cell.id) || [];
