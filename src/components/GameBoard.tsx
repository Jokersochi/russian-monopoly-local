...
export const GameBoard = () => {
  ...
  // Pre-compute cell ownership map for O(1) cell lookup
  ...
  // Pre-compute players per cell position map for O(1) cell lookup
  ...
  // Pre-compute net worth standings using useMemo to eliminate O(P * C) array filtering during renders
  const standings = useMemo(() => {
    if (!gameState) return [];
    return [...activePlayers]
      .map((p, i) => {
        const propVal = cells.filter(c => p.properties.includes(c.id)).reduce((s, c) => s + (c.price || 0), 0);
        const houseVal = cells.filter(c => p.properties.includes(c.id)).reduce((s, c) => s + (activeHouses[c.id] || 0) * (c.houseCost || 0), 0);
        return { p, i, net: p.money + propVal + houseVal };
      })
      .sort((a, b) => b.net - a.net);
  }, [gameState, activePlayers, activeHouses, cells]);
