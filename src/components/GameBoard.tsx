...
export const GameBoard = () => {
  ...
  // Precalculate property owners, player board positions, and player net worth in a single O(P) pass
  // to avoid O(P * C) linear array scans on every render frame during game animations.
  const { ownerMap, playersOnCellMap, standings } = useMemo(() => {
    const ownerMap = new Map<number, (NonNullable<typeof gameState>['players'])[0]>();
    const playersOnCellMap = new Map<number, NonNullable<typeof gameState>['players']>();
    ...
    for (const player of players) {
      ...
      const list = playersOnCellMap.get(player.position) || [];
      list.push(player);
      playersOnCellMap.set(player.position, list);
    }
      ...
      .map((p, i) => {
        ...
        for (const propId of p.properties) {
          ...
          if (cell) {
            ...
            houseVal += (houses[propId] || 0) * (cell.houseCost || 0);
          }
        }
    ...
    return { ownerMap, playersOnCellMap, standings };
  }, [gameState, cells]);
