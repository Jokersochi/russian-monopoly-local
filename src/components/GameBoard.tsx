...
export const GameBoard = () => {
  ...
  // Pre-compute player cell locations to avoid O(C * P) array filtering on every render
  ...
  // Pre-compute net worth standings to eliminate inline array sorting and multi-pass filtering
  const netWorthStandings = useMemo(() => {
    const cellValueMap = new Map<number, { price: number; houseCost: number }>();
    cells.forEach((c) => {
      cellValueMap.set(c.id, { price: c.price || 0, houseCost: c.houseCost || 0 });
    });
    ...
    return [...players]
      .map((p, i) => {
        ...
        let houseVal = 0;
        p.properties.forEach((cellId) => {
          const info = cellValueMap.get(cellId);
          if (info) {
            propVal += info.price;
            houseVal += (houses[cellId] || 0) * info.houseCost;
          }
        });
