...
export const GameBoard = () => {
  ...
  // Precalculate cell ownership, players on cell, and net worth standings in $O(1)$ lookups
  const { ownerMap, playersByCellMap, standings } = useMemo(() => {
    const ownerMap = new Map<number, NonNullable<typeof players>[0]>();
    const playersByCellMap = new Map<number, NonNullable<typeof players>[0][]>();

    if (!gameState) {
      return { ownerMap, playersByCellMap, standings: [] };
    }
    ...
    for (const p of players) {
      ...
      const existing = playersByCellMap.get(p.position);
      if (existing) {
        existing.push(p);
      } else {
        playersByCellMap.set(p.position, [p]);
      }
    }
      ...
      <div className="relative bg-gradient-board rounded-2xl shadow-board p-8 border-8 border-russia-gold backdrop-blur-sm">
        ...
        <div className="relative" style={{ width: '902px', height: '902px' }}>
          {cells.map((cell) => {
            ...
            const playersHere = playersByCellMap.get(cell.id) || [];
