...
export const GameBoard = () => {
  ...
  // Memoize cell ownership map O(1) lookup across 40 board cells
  ...
  // Memoize player locations map O(1) lookup across 40 board cells
  ...
  // Memoize player net worth standings computation to avoid re-filtering cells on every render frame
  const standings = useMemo(() => {
    ...
    return [...gameState.players]
      .map((p, i) => {
        let propVal = 0;
        let houseVal = 0;
        const propSet = new Set(p.properties);
        for (const cell of cells) {
          if (propSet.has(cell.id)) {
            propVal += cell.price || 0;
            houseVal += (gameState.houses[cell.id] || 0) * (cell.houseCost || 0);
          }
        }
