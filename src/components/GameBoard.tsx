...
export const GameBoard = () => {
  ...
  // Performance optimization: Pre-calculate property ownership and player positions
  // to avoid O(N*M) lookups inside the render loop.
  const { propertyOwners, playerPositions } = useMemo(() => {
    const ownersMap = new Map<number, Player>();
    const positionsMap = new Map<number, Player[]>();

    if (!gameState) return { propertyOwners: ownersMap, playerPositions: positionsMap };

    gameState.players.forEach(player => {
      // Map properties to owners
      ...
      if (!player.bankrupt) {
        const playersOnCell = positionsMap.get(player.position) || [];
        playersOnCell.push(player);
        positionsMap.set(player.position, playersOnCell);
      }
    });

    return { propertyOwners: ownersMap, playerPositions: positionsMap };
  }, [gameState]);
