...
export const GameBoard = () => {
  ...
  // Performance optimization: Precalculate property ownership, player cell positions,
  // and net worth standings in O(1) Map lookups to avoid O(P * C) array traversals on every render frame.
  const { playerPositionsMap, propertyOwnersMap, netWorthStandings } = useMemo(() => {
    if (!gameState) {
      return {
        playerPositionsMap: new Map<number, Player[]>(),
        propertyOwnersMap: new Map<number, Player>(),
        netWorthStandings: [],
      };
    }
    ...
    const playerPosMap = new Map<number, Player[]>();
    const propOwnerMap = new Map<number, Player>();

    // Map cell position -> list of players on that cell
    players.forEach((player) => {
      const existing = playerPosMap.get(player.position);
      if (existing) {
        existing.push(player);
      } else {
        playerPosMap.set(player.position, [player]);
      }

      // Map property cell ID -> owner player
      player.properties.forEach((propId) => {
        propOwnerMap.set(propId, player);
      });
    });

    // Calculate net worth standings in a single pass over players
    const standings = players
      .map((p, i) => {
        let propVal = 0;
        let houseVal = 0;
        p.properties.forEach((pid) => {
          // O(1) fast path lookup by direct index if array ordering matches ID, falling back to find
    ...
    return {
      playerPositionsMap: playerPosMap,
      propertyOwnersMap: propOwnerMap,
      netWorthStandings: standings,
    };
  }, [gameState, cells]);
  ...
  const getOwner = (cellId: number) => propertyOwnersMap.get(cellId);
  const getPlayersOnCell = (cellId: number) => playerPositionsMap.get(cellId) || [];
