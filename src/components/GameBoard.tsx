...
export const GameBoard = () => {
  ...
  const movedPlayerKey = gameState?.players.map(p => `${p.id}:${p.position}`).join(',') ?? '';
  ...
  // Pre-compute cell owners, players on cells, and net worth standings in O(P + C) single pass
  const { ownerByCellId, playersByCellId, netWorthStandings } = useMemo(() => {
    const ownerMap = new Map<number, Player>();
    const playersMap = new Map<number, Player[]>();
    const netWorthMap = new Map<number, number>();

    if (!gameState) {
      return { ownerByCellId: ownerMap, playersByCellId: playersMap, netWorthStandings: [] };
    }
    ...
    for (const player of players) {
      ...
      for (const propId of player.properties) {
        ...
        const c = cells[propId];
        if (c) {
          propVal += c.price || 0;
          houseVal += (houses[propId] || 0) * (c.houseCost || 0);
        }
      }

      netWorthMap.set(player.id, player.money + propVal + houseVal);

      const posList = playersMap.get(player.position) || [];
      posList.push(player);
      playersMap.set(player.position, posList);
    }

    const standings = [...players]
      .map((p, i) => ({ p, i, net: netWorthMap.get(p.id) ?? p.money }))
      .sort((a, b) => b.net - a.net);

    return {
      ownerByCellId: ownerMap,
      playersByCellId: playersMap,
