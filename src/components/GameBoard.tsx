...
export const GameBoard = () => {
  ...
  // Performance optimization: Pre-calculate property ownership and player positions using Maps.
  // This reduces complexity from O(Cells * Players) to O(Cells + Players) per render.
  ...
  const playersOnCells = useMemo(() => {
    ...
    gameState.players.forEach(player => {
      ...
      const players = positionsMap.get(player.position) || [];
      players.push(player);
      positionsMap.set(player.position, players);
    });
    return positionsMap;
  }, [gameState]);
