...
export const GameBoard = () => {
  ...
  // Destructure players and houses with defaults so hooks can run unconditionally
  const { players = [], houses = {} } = gameState || {};

  // Pre-compute cell ownership map to avoid O(C * P) array scans on each render
  ...
  // Pre-compute players on each cell to convert O(C * P) filtering to O(1) map lookups
  ...
  // Pre-compute net worth standings to prevent O(P * C) property & house filtering
