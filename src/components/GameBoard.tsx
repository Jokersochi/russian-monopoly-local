...
export const GameBoard = () => {
  ...
  // Precalculate property owners, players per cell, and net worth standings with O(1) lookups
  const { ownerMap, playersByCell, standings } = useMemo(() => {
    const ownerMap = new Map<number, (typeof players)[0]>();
    const playersByCell = new Map<number, typeof players>();
    ...
    players.forEach((p) => {
      ...
      const onCell = playersByCell.get(p.position);
      if (onCell) {
        onCell.push(p);
      } else {
        playersByCell.set(p.position, [p]);
      }
    });
    ...
    const standings = players
      .map((p, i) => {
        let propVal = 0;
        let houseVal = 0;
        for (let k = 0; k < p.properties.length; k++) {
          const propId = p.properties[k];
          const cell = cellMap.get(propId);
          if (cell) {
    ...
    return { ownerMap, playersByCell, standings };
