...
export const GameBoard = () => {
  ...
  const cpIdx = gameState?.currentPlayer || 0;
  ...
  // Performance: Map-based lookups for O(1) cell access during render
  const propertyOwners = useMemo(() => {
    const map = new Map<number, NonNullable<typeof players>[0]>();
    if (!players) return map;
    players.forEach(p => {
      p.properties.forEach(id => map.set(id, p));
    });
    return map;
  }, [players]);
  ...
  // Performance: Memoized standings calculation (O(Players * AvgProperties) vs O(Players * Cells))
  const playerStandings = useMemo(() => {
    if (!players || !houses) return [];
    return [...players]
      .map((p) => {
        let propVal = 0;
        let houseVal = 0;
        p.properties.forEach(id => {
          const cell = cells[id]; // Optimization: board cells are ordered by ID
          if (cell) {
            propVal += cell.price || 0;
            houseVal += (houses[id] || 0) * (cell.houseCost || 0);
          }
        });
