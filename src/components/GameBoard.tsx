...
export const GameBoard = () => {
  ...
  // Precalculate cell ownership, player cell positions, and net worth standings to avoid O(P x C) lookups per render frame
  const { players = [], houses = {} } = gameState || {};
  const { cellOwnerMap, playersOnCellMap, netWorthStandings } = useMemo(() => {
    const ownerMap = new Map<number, NonNullable<typeof players>[0]>();
    const onCellMap = new Map<number, NonNullable<typeof players>[0][]>();

    players.forEach((p) => {
      const existing = onCellMap.get(p.position) || [];
      existing.push(p);
      onCellMap.set(p.position, existing);
    ...
    const standings = players
      .map((p, i) => {
        ...
        let houseVal = 0;
        p.properties.forEach((pid) => {
          // Fast path O(1) array index lookup with fallback
    ...
    <>
      <div className="relative bg-gradient-board rounded-2xl shadow-board p-8 border-8 border-russia-gold backdrop-blur-sm">
        ...
        <div className="relative" style={{ width: '902px', height: '902px' }}>
          {cells.map((cell) => {
            const owner = cellOwnerMap.get(cell.id);
