...
export const GameBoard = () => {
  ...
  const playerPositionsKey = gameState?.players.map(p => `${p.id}:${p.position}`).join(',') ?? '';
  ...
  // Performance optimization: Pre-compute property owners, players on cells, and net worth standings
  // in a single useMemo pass to avoid O(Players × Cells) calculations on every render.
  const { propertyOwnerMap, playersOnCellMap, standings } = useMemo(() => {
    if (!gameState) {
      return {
        propertyOwnerMap: new Map<number, Player>(),
        playersOnCellMap: new Map<number, Player[]>(),
        standings: [],
      };
    }

    const ownerMap = new Map<number, Player>();
    const posMap = new Map<number, Player[]>();

    gameState.players.forEach((player) => {
      // Map property ownership
      player.properties.forEach((cellId) => {
        ownerMap.set(cellId, player);
      });

      // Map player positions
      const existing = posMap.get(player.position) || [];
      existing.push(player);
      posMap.set(player.position, existing);
    });

    // Compute net worth standings (direct array indexing for cells since BOARD_CELLS IDs 0-39 match indices)
    const rankedStandings = gameState.players
      .map((p) => {
        let propVal = 0;
        let houseVal = 0;
        p.properties.forEach((cellId) => {
          const cell = cells[cellId] || cells.find((c) => c.id === cellId);
          if (cell) {
            propVal += cell.price || 0;
            houseVal += (gameState.houses[cellId] || 0) * (cell.houseCost || 0);
          }
        });
