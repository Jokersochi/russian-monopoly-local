...
export const GameBoard = () => {
  ...
  // Memoized maps and standings calculations at top level before conditional returns
  const { ownersMap, playersOnCellMap, sortedStandings } = useMemo(() => {
    if (!gameState) {
      return {
        ownersMap: new Map<number, Player>(),
    ...
    const playersOnCell = new Map<number, Player[]>();

    players.forEach((p) => {
      // Build player positions map
      const existing = playersOnCell.get(p.position) || [];
      existing.push(p);
      playersOnCell.set(p.position, existing);

      // Build cell owner map
      p.properties.forEach((propId) => {
        owners.set(propId, p);
      });
    });

    // Compute player net worths and sort
    const standings = players
      .map((p) => {
        ...
        p.properties.forEach((propId) => {
          // Fast path cell lookup using direct index array lookup with fallback
    ...
    return {
      ownersMap: owners,
      playersOnCellMap: playersOnCell,
    ...
    <>
      <div className="relative bg-gradient-board rounded-2xl shadow-board p-8 border-8 border-russia-gold backdrop-blur-sm">
        ...
        <div className="relative" style={{ width: '902px', height: '902px' }}>
          {cells.map((cell) => {
            const owner = ownersMap.get(cell.id);
