...
export const GameBoard = () => {
  ...
  // Pre-calculate property owners for O(1) lookup during render
  // This avoids O(Players * Properties) search for each of the 40 cells
  ...
  // Pre-calculate player positions for O(1) lookup during render
  // This avoids O(Players) filter for each of the 40 cells
  const playersOnCells = useMemo(() => {
    const map = new Map<number, Player[]>();
    gameState?.players.forEach(player => {
      if (player.bankrupt) return;
      const players = map.get(player.position) || [];
      players.push(player);
      map.set(player.position, players);
    });
    return map;
  }, [gameState?.players]);
