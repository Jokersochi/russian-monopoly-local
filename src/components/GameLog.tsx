...
export const GameLog = () => {
  ...
  // Single-pass optimization: aggregate log counts and build reversed filtered list in a single O(N) backward traversal
  const { filtered, counts, total } = useMemo(() => {
    ...
    if (!gameState) {
      return { filtered: [] as LogEntry[], counts, total: 0 };
    }
    ...
    return { filtered, counts, total: gameLog.length };
  }, [gameState, filter]);
