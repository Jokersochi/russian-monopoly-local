      ...
      <div className="p-3 border-b border-russia-gold/20 flex items-center justify-between gap-2 flex-shrink-0">
        ...
        <div className="flex gap-1">
          {(Object.entries(FILTER_LABELS) as [Filter, string][]).map(([key, label]) => {
            const count = key !== 'all' ? countByType(key as LogEntry['type']) : 0;
            const tooltipText = key === 'all' ? 'Все события' : key;
            const ariaLabelText = key === 'all' ? tooltipText : `${tooltipText}, ${count} событий`;
            return (
              <Tooltip key={key}>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      'h-6 px-1.5 text-xs focus-visible:ring-2 focus-visible:ring-offset-1 focus-visible:ring-russia-gold',
