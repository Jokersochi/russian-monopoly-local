...
export const GameLog = () => {
  ...
  // Single-pass computation memoized to avoid multi-pass array allocations on every render
  const { filtered, counts } = useMemo(() => {
    const gameLog = gameState?.gameLog;
    if (!gameLog || gameLog.length === 0) {
      return {
        filtered: [] as LogEntry[],
        counts: { success: 0, info: 0, warning: 0, error: 0 } as Record<LogEntry['type'], number>,
      };
    }
    ...
    // Reverse order traversal in a single O(N) pass
      ...
      <div className="p-3 border-b border-russia-gold/20 flex items-center justify-between gap-2 flex-shrink-0">
        ...
        <div className="flex gap-1">
          {(Object.entries(FILTER_LABELS) as [Filter, string][]).map(([key, label]) => {
            const count = key === 'all' ? gameState.gameLog.length : counts[key as LogEntry['type']] || 0;
            return (
              <Button
                key={key}
                size="sm"
