...
export const GameLog = () => {
  ...
  const { filtered, counts, totalCount } = useMemo(() => {
    ...
    if (!log) {
      return { filtered: [], counts, totalCount: 0 };
    }

    // Performance Optimization:
    // Single-pass O(N) backward traversal to build reversed/filtered list
    // and calculate exact type counts without allocating multiple intermediate arrays
    // or executing repeated O(N) filter passes per filter button render.
    ...
    return { filtered, counts, totalCount: log.length };
      ...
      <div className="p-3 border-b border-russia-gold/20 flex items-center justify-between gap-2 flex-shrink-0">
        ...
        <div className="flex gap-1">
          {(Object.entries(FILTER_LABELS) as [Filter, string][]).map(([key, label]) => {
            const count = key === 'all' ? totalCount : counts[key as LogEntry['type']];
            return (
              <Button
                key={key}
                size="sm"
