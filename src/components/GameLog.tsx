...
export const GameLog = () => {
  ...
  // Performance optimization: Calculate filtered entries and counts in a single O(N) backward pass.
  // This avoids allocating multiple temporary arrays (.slice().reverse(), .filter()) and
  // eliminates repeated full-array traversals for filter button counts on every render frame.
      ...
      <div className="p-3 border-b border-russia-gold/20 flex items-center justify-between gap-2 flex-shrink-0">
        ...
        <div className="flex gap-1">
          {(Object.entries(FILTER_LABELS) as [Filter, string][]).map(([key, label]) => {
            const count = key === 'all' ? gameState.gameLog.length : typeCounts[key as LogEntry['type']] || 0;
            return (
              <Button
                key={key}
                size="sm"
