      ...
      <div className="p-3 border-b border-russia-gold/20 flex items-center justify-between gap-2 flex-shrink-0">
        ...
        <div className="flex gap-1">
          {(Object.entries(FILTER_LABELS) as [Filter, string][]).map(([key, label]) => {
            const isAll = key === 'all';
            const tooltipText = isAll ? 'Все события' : key;
            const btnLabel = `${isAll ? 'Показать' : 'Фильтр:'} ${tooltipText}`;
            return (
              <Tooltip key={key}>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      ...
                      !isAll && countByType(key as LogEntry['type']) === 0 && 'opacity-30'
                    )}
                  >
                    {label}
                    {!isAll && countByType(key as LogEntry['type']) > 0 && (
