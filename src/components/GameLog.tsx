...
export const GameLog = () => {
  ...
  // Single-pass memoized reversal and per-type event counting O(N)
  const { entries, counts } = useMemo(() => {
    if (!gameState?.gameLog) {
      return { entries: [], counts: { success: 0, info: 0, warning: 0, error: 0 } };
    }
    const log = gameState.gameLog;
    const rev: LogEntry[] = new Array(log.length);
    const cnt = { success: 0, info: 0, warning: 0, error: 0 };

    for (let i = log.length - 1, j = 0; i >= 0; i--, j++) {
      const entry = log[i];
      rev[j] = entry;
      if (entry.type in cnt) {
        cnt[entry.type as keyof typeof cnt]++;
      }
    }

    return { entries: rev, counts: cnt };
  }, [gameState?.gameLog]);
      ...
      <div className="p-3 border-b border-russia-gold/20 flex items-center justify-between gap-2 flex-shrink-0">
        ...
        <div className="flex gap-1">
          {(Object.entries(FILTER_LABELS) as [Filter, string][]).map(([key, label]) => {
            const count = key === 'all' ? 0 : counts[key as keyof typeof counts] || 0;
            return (
              <Button
                key={key}
                size="sm"
