...
export const GameLog = () => {
  ...
  // Perform single O(N) backward pass to reverse logs, aggregate type counts, and filter in one go
  ...
  const getCount = (type: LogEntry['type']) => counts[type] || 0;
          ...
          {(Object.entries(FILTER_LABELS) as [Filter, string][]).map(([key, label]) => (
            <Button
              ...
              className={cn(
                ...
                key !== 'all' && getCount(key as LogEntry['type']) === 0 && 'opacity-30'
              )}
              title={key === 'all' ? 'Все события' : key}
            >
              {label}
              {key !== 'all' && getCount(key as LogEntry['type']) > 0 && (
                <span className="ml-0.5 opacity-70">{getCount(key as LogEntry['type'])}</span>
              )}
            </Button>
          ))}
        </div>
