...
export const GameLog = () => {
  ...
  // Bolt Optimization: Consolidate reverse traversal, type counting, and filtering
  // into a single O(N) backward pass inside useMemo to avoid 5+ redundant array
  // allocations (.slice(), .reverse(), and multiple .filter() calls) per render frame.
      ...
      <div className="p-3 border-b border-russia-gold/20 flex items-center justify-between gap-2 flex-shrink-0">
        ...
        <div className="flex gap-1">
          {(Object.entries(FILTER_LABELS) as [Filter, string][]).map(([key, label]) => {
            const count = key === 'all' ? 0 : typeCounts[key as LogEntry['type']];
            return (
              <Button
                key={key}
                size="sm"
