...
export const GameLog = () => {
  ...
  // Bolt optimization: Single-pass backward traversal to aggregate counts and reversed filtered entries
  // Replaces multiple O(N) calls (slice, reverse, filter, and multiple countByType filters) with O(N) single pass.
  const { filteredEntries, typeCounts, totalCount } = useMemo(() => {
    const gameLog = gameState?.gameLog;
    if (!gameLog || gameLog.length === 0) {
      return { filteredEntries: [], typeCounts: {} as Record<LogEntry['type'], number>, totalCount: 0 };
    }
    ...
    // Iterate backwards to place newest entries first without Array.reverse() allocation
    ...
    return { filteredEntries, typeCounts, totalCount: gameLog.length };
  }, [gameState, filter]);
