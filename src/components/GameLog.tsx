...
export const GameLog = () => {
  ...
  // Single-pass optimization: consolidate log array reversal, type filtering,
  // and type counts into a single O(N) backward loop instead of multiple array passes.
  const { filteredLogs, counts } = useMemo(() => {
    ...
    if (!log) return { filteredLogs, counts };
      ...
      <div className="p-3 border-b border-russia-gold/20 flex items-center justify-between gap-2 flex-shrink-0">
        ...
        <div className="flex gap-1">
          {(Object.entries(FILTER_LABELS) as [Filter, string][]).map(([key, label]) => {
            const count = key === 'all' ? gameState.gameLog.length : counts[key as LogEntry['type']];
            return (
              <Button
                key={key}
                size="sm"
