...
export const GameLog = () => {
  ...
  // Single-pass O(N) backward loop: reverses, counts by type, and filters in 1 pass instead of 10+ array allocations/traversals
  const { filteredEntries, counts, totalCount } = useMemo(() => {
    ...
    return { filteredEntries: filtered, counts, totalCount: log.length };
  }, [gameState, filter]);
      ...
      <div className="p-3 border-b border-russia-gold/20 flex items-center justify-between gap-2 flex-shrink-0">
        ...
        <div className="flex gap-1">
          {(Object.entries(FILTER_LABELS) as [Filter, string][]).map(([key, label]) => {
            const count = key === 'all' ? totalCount : counts[key as LogEntry['type']] || 0;
            return (
              <Button
                key={key}
                size="sm"
