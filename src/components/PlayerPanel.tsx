...
export const PlayerPanel = () => {
  ...
  // Precompute O(1) cell lookup map for property calculations
  ...
  // Performance optimization: Memoize player financial and property calculations
  // to avoid O(P * C) array filtering on every render frame or accordion toggle
  const playerStats = useMemo(() => {
    ...
    return gameState.players.map(player => {
      ...
      for (const id of player.properties) {
        ...
        const hc = gameState.houses[id] || 0;
        if (hc > 0) {
          houseValue += hc * (cell.houseCost || 0);
        }

        if (player.mortgaged.includes(id)) {
